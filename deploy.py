...
def get_keywords(text,n):
    words = KeywordExtractor(top=n)
    words_list = [x[0] for x in words.extract_keywords(text)]
    return words_list
...
if radio=="Top 10":
    result = get_keywords(input_review, 10)
    for word in result:
        st.markdown(word)
elif radio=="Top 20":
    result = get_keywords(input_review, 20)
    for word in result:
        st.markdown(word)
else:
    result = get_keywords(input_review, 50)
    for word in result:
        st.markdown(word)